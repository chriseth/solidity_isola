...
Another example is contract \code{Token} from Sec.~\ref{section:solidity}.
%
...
%
We can see that the state variable \code{x} is assigned parameter \code{y}
received by the constructor, having the constraint that $y < 1000$.
%
Since no other function modifies \code{x}, we can see that $x < 1000$ is an
invariant.
%
Without this invariant, the SMT component reports a potential overflow in
function \code{plusX}, and is not able to prove the assertion.
%
The invariant can be represented as an extra \code{require(x < 1000);} in the
beginning of function \code{plusX}, which then leads to no overflow being
